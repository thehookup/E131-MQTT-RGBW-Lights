...
#define MQTT_CLIENT_NAME    "[MQTT CLIENT]" //This name will be used for all MQTT Messages, must be unique.
const char* ssid = "[WIFI-2G-SSID]";
  ...
  while (!client.connected()) {
    if(retries < 5)
    {
      ...
      if (client.connect(mqtt_client_name, mqtt_user, mqtt_pass)) 
      {
        ...
        client.subscribe(MQTT_CLIENT_NAME"/color_temp");
...
void callback(char* topic, byte* payload, unsigned int length) 
{
  ...
  if (newTopic == MQTT_CLIENT_NAME "/color_temp")
  {
    client.publish(MQTT_CLIENT_NAME "/color_tempState", charPayload); 
    t = intPayload;
    updateLights();
  }
...
void updateLights()
{
      if (t<= 413)
      {
        cw = (map(t,413,153,0,255) * w) / 255;
      }else{
        cw=0;
      }
      if (t>= 240)
      {
        ww = (map(t,240,500,0,255) * w) / 255;
      }else{
        ww=0;
      }
